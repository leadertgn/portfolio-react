...
export default function Services() {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.VITE_BACKEND_URL || 'http://localhost:5000'}/api/services`)
      .then((res) => {
        const apiServices = Array.isArray(res.data) ? res.data : []
        setServices(apiServices.length > 0 ? apiServices : localServices)
        setLoading(false)
      })
      .catch((err) => {
        console.error(err)
